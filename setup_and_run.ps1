...
# Helper: envoie un message au relay server si configure
function Send-ToRelay {
    param([string]$msg)
    if (-not $RELAY_URL) { return }
    try {
        $body = "[causality_model] $env:COMPUTERNAME`n$msg"
        Invoke-WebRequest -Uri $RELAY_URL -Method POST -Body $body -UseBasicParsing -TimeoutSec 3 | Out-Null
    } catch { }
}
...
if (-not $installed) {
    $errMsg = "ERREUR installation Python: echec automatique. Verifiez la connexion internet."
    Write-Host ""
...
if ($LASTEXITCODE -ne 0) {
    $errMsg = "ERREUR venv creation failed (exit $LASTEXITCODE)"
    Write-Host "  ERROR: venv creation failed (exit $LASTEXITCODE)." -ForegroundColor Red
    Send-ToRelay $errMsg
    Read-Host "Press Enter to exit"; exit 1
}
if (-not (Test-Path $venvPython)) {
    $errMsg = "ERREUR venv Python introuvable: $venvPython"
...
if ($LASTEXITCODE -ne 0) {
    $errMsg = "ERREUR installation des dependances pip (exit $LASTEXITCODE)"
